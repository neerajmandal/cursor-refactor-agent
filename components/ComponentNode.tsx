...
function ComponentNodeInner({ data, selected }: NodeProps<ComponentFlowNode>) {
  ...
  return (
    <div
      aria-busy={running}
      className={`flex h-[52px] w-[180px] items-center justify-center gap-2 border bg-node px-3 ${
        selected
          ? "border-ink"
          : running
            ? "border-accent"
            : data.emphasized
              ? "border-[#3d5a80] shadow-[inset_0_0_0_1px_#3d5a80]"
              : "border-line"
      }`}
    >
      <Handle type="target" position={Position.Top} className="!h-px !w-px !border-0 !bg-transparent !opacity-0" />
      {running ? (
        <span className="node-spinner shrink-0" aria-hidden />
      ) : null}
